...
void *csonDecodeObject(cJSON *json, CsonModel *model, int modelSize)
{
    ...
    for (short i = 0; i < modelSize; i++)
    {
        printf(" (model[%d].type:%d , model[%d].offset:%d\r\n", i, model[i].type, i, model[i].offset);
        switch (model[i].type)
        {
        case CSON_TYPE_CHAR:
        printf(" CSON_TYPE_CHAR: %c \r\n", (char)csonDecodeNumber(json, model[i].key));
            *(char *)((int)obj + model[i].offset) = (char)csonDecodeNumber(json, model[i].key);
            
            break;
        case CSON_TYPE_SHORT:
            printf(" CSON_TYPE_SHORT: %d \r\n", (short)csonDecodeNumber(json, model[i].key));

            *(short *)((int)obj + model[i].offset) = (short)csonDecodeNumber(json, model[i].key);

            break;
        case CSON_TYPE_INT:
            printf(" CSON_TYPE_INT: %d \r\n", (int)csonDecodeNumber(json, model[i].key));

            *(int *)((int)obj + model[i].offset) = (int)csonDecodeNumber(json, model[i].key);

            break;
        case CSON_TYPE_LONG:
            printf(" CSON_TYPE_LONG: %d \r\n", (int)csonDecodeNumber(json, model[i].key));
        ...
        case CSON_TYPE_BOOL:
            *(char *)((int)obj + model[i].offset) = (char)csonDecodeBool(json, model[i].key);
            break;
        case CSON_TYPE_STRING:
            printf(" CSON_TYPE_STRING: %s \r\n", (char *)csonDecodeString(json, model[i].key));

            *(int *)((int)obj + model[i].offset) = (char *)csonDecodeString(json, model[i].key);
            break;
        case CSON_TYPE_LIST:
